...
`crew_id`, `nickname` 두 개의 컬럼으로 구성한다.

**1-3. crew 테이블에 들어가야 할 크루들의 정보는 어떻게 추출할까? (hint: DISTINCT) → SELECT**

```sql
SELECT DISTINCT crew_id, attendance.nickname
FROM attendance;
```

**1-4. 최종적으로 crew 테이블 생성 → CREATE**
...
**1-5. attendance 테이블에서 크루 정보를 추출해서 crew 테이블에 삽입하기 → INSERT**
...
**2-2. 컬럼을 삭제하려면 어떻게 해야 하는가? → ALTER**
...
**`attendance.crew_id` → `crew.crew_id` 외래키 설정 → ALTER**
...
**crew 테이블의 nickname에 유니크 키 설정 → ALTER**
...
**어셔의 3월 6일 출석 기록이 누락됐는지 확인 → SELECT**
...
**어셔의 3월 6일 출석 기록 추가 (09:31 등교, 18:01 하교) → INSERT**
...
**주니의 3월 12일 등교 시각을 10:05 → 10:00으로 수정 → UPDATE**
...
**crew 테이블에서 crew_id 기준으로 nickname을 가져와서 출석 정보와 함께 조회 → SELECT (JOIN)**
...
> 검프는 SQL이 익숙지 않아 crew 테이블에서 먼저 닉네임을 검색하고 해당 아이디 값을 찾아 직접 `WHERE`문에서 `crew_id` 항목의 값을 수동으로 입력했다. 그런데 nickname을 입력하면 이를 기준으로 쿼리문을 처리할 수도 있지 않을까?

**nickname을 입력하면 이를 기준으로 출석 기록을 조회하는 쿼리 → SELECT (서브쿼리)**
...
**3월 5일 가장 늦게 하교한 크루의 닉네임과 하교 시각 조회 → SELECT**
...
**크루별 기록된 날짜 수 조회 → SELECT (GROUP BY + COUNT)**
...
**`start_time IS NOT NULL` 조건으로 크루별 등교 기록 날짜 수 조회 → SELECT**
...
**날짜별 등교한 크루 수 조회 → SELECT**

```sql
SELECT attendance_date, COUNT(nickname)
FROM attendance a
JOIN crew c ON a.crew_id = c.crew_id
...
**크루별 가장 빠른 등교 시각(MIN)과 가장 늦은 등교 시각(MAX) 조회 → SELECT**

```sql
SELECT nickname, MIN(start_time), MAX(start_time)
FROM attendance a
JOIN crew c ON a.crew_id = c.crew_id
GROUP BY c.crew_id;
```